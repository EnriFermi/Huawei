...
public:
    ...
    {
        char last_p_grup = arr[0][K];
        char last_s_grup = arr[1][K];
        if ((last_p_grup == last_s_grup) && (last_p_grup != 0))
        {
            char tmp_arr[L];
            memset(tmp_arr, 0, L);
            while (K >= 0)
            {
                if (arr[0][K] == last_p_grup)
                {
                    char b = L_P_arr[K];
        ...
        if ((last_s_grup == 0) && (last_p_grup != last_s_grup))
        {
            char tmp_arr[L];
            memset(tmp_arr, 0, L);
            while (K >= 0)
